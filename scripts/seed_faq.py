...
# Insert FAQs with dummy embeddings, skipping duplicates
db = SessionLocal()
for item in faq_database:
    exists = db.query(FAQ).filter(FAQ.question == item["question"]).first()
    if exists:
...
db.commit()
db.close()
print("FAQ database seeded with dummy embeddings (duplicates skipped).")
